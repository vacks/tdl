...
type Dashboard = { telegram: { status: string }; downloads: { active: number; completedItems: number; failedItems: number }; upstreamVersion: string; system: { current: Point; trend: Point[] } }
const router = useRouter(); const dashboard = ref<Dashboard>(); const username = ref(''); let timer: number | undefined
async function load() { dashboard.value = await api<Dashboard>('/api/dashboard') }
...
<template>
  <el-container class="shell"><el-aside width="244px"><div class="brand">tdl <span>WEB</span></div><el-menu default-active="/" router><el-menu-item index="/">仪表盘</el-menu-item><el-menu-item index="/accounts">Telegram 账户</el-menu-item><el-menu-item index="/downloads">下载管理</el-menu-item><el-menu-item index="/settings">功能配置</el-menu-item></el-menu><div class="sidebar-meta"><span>上游 tdl <b>v0.20.4</b></span><span>项目版本 <b>v0.1.0</b></span></div></el-aside>
    <el-container><el-header><span>仪表盘</span><div><span class="subtle">{{ username }}</span><el-button text @click="logout">退出</el-button></div></el-header><el-main><main class="dashboard-page"><section class="dashboard-hero"><div><p class="eyebrow">OVERVIEW</p><h1>运行概览</h1><p class="subtle">查看服务资源与下载状态。</p></div><div class="live-pill"><i></i> 实时监控</div></section>
      <section class="overview-grid"><article class="overview-card accent-blue"><span>Telegram 账户</span><strong>{{ dashboard?.telegram.status === 'not_connected' ? '尚未登录' : '已连接' }}</strong><small>{{ dashboard?.telegram.status === 'not_connected' ? '请先在账户页完成登录' : '当前服务可访问 Telegram' }}</small></article><article class="overview-card accent-violet"><span>进行中下载</span><strong>{{ dashboard?.downloads.active ?? 0 }}</strong><small>排队、下载或暂停中</small></article><article class="overview-card accent-green"><span>累计完成</span><strong>{{ dashboard?.downloads.completedItems ?? 0 }}</strong><small>已完成下载</small></article><article class="overview-card accent-orange"><span>下载失败</span><strong>{{ dashboard?.downloads.failedItems ?? 0 }}</strong><small>可在下载管理中查看和重试</small></article></section>
