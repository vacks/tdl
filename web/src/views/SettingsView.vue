...
// The backend deliberately removes presentation selectors so Telegram and the
// listener compare equivalent reactions consistently (❤ and ❤️). Restore an
// emoji presentation selector only for display, leaving the stored value and
// de-duplication behaviour unchanged.
function displayReactionEmoji(value: string) { return value.replace(/(\p{Emoji})(?!\ufe0e|\ufe0f|\u20e3|[\u{1f3fb}-\u{1f3ff}])/gu, (emoji) => /\p{Emoji_Presentation}/u.test(emoji) ? emoji : `${emoji}\ufe0f`) }
  ...
  <el-container class="shell">
    ...
    <el-container><el-header><span>配置管理</span><el-button text @click="logout">退出管理台</el-button></el-header>
      <el-main class="settings-page"><section class="page-heading settings-heading"><div><p class="eyebrow">SETTINGS</p><h1>配置管理</h1></div><el-button type="primary" :loading="saving" @click="save">保存配置</el-button></section>
        ...
        <el-card shadow="never" class="settings-card"><template #header><strong>Bot 控制</strong></template><el-form label-position="top" @submit.prevent><el-form-item label="启用 Bot 控制"><div style="display:flex;align-items:center;gap:12px"><el-switch v-model="form.bot.enabled" /><span>{{ form.bot.enabled ? '已启用' : '已关闭' }}</span></div></el-form-item><el-form-item label="Bot Token"><el-input v-model="form.bot.token" type="password" show-password placeholder="123456:ABC..." autocomplete="off" /></el-form-item><el-form-item label="控制用户 ID"><div class="id-tag-input"><el-tag v-for="id in userIDs" :key="id" closable @close="removeUserID(id)">{{ id }}</el-tag><el-input v-model="userIDInput" placeholder="添加 Telegram 用户 ID" @keydown.enter.prevent="addUserIDs" /></div></el-form-item><p class="field-help">填写允许控制 Bot 的 Telegram 数字用户 ID；可点击标签右侧的 × 删除。可向 <code>@userinfobot</code> 发送 <code>/start</code> 查询自己的 ID。</p><el-form-item label="通知类型"><div style="display:flex;flex-wrap:wrap;gap:24px"><el-checkbox v-model="form.bot.notifications.taskCreated">任务创建</el-checkbox><el-checkbox v-model="form.bot.notifications.taskCompleted">下载完成</el-checkbox><el-checkbox v-model="form.bot.notifications.taskPartial">部分完成</el-checkbox><el-checkbox v-model="form.bot.notifications.taskFailed">下载失败</el-checkbox></div></el-form-item><el-alert type="info" :closable="false" title="可用命令：/help、/tasks、/chats、/status、/config、/restart；也可直接发送 Telegram 消息链接创建任务。" /></el-form></el-card>
        <el-card shadow="never" class="settings-card"><template #header><strong>表情触发下载</strong></template><el-form label-position="top" @submit.prevent><el-form-item label="启用监听"><div style="display:flex;align-items:center;gap:12px"><el-switch v-model="form.reaction.enabled" /><span>{{ form.reaction.enabled ? '已启用' : '已关闭' }}</span></div></el-form-item><el-form-item label="触发表情"><div class="id-tag-input"><el-tag v-for="emoji in reactionEmojis" :key="emoji" closable @close="removeReactionEmoji(emoji)">{{ displayReactionEmoji(emoji) }}</el-tag><el-input v-model="reactionEmojiInput" placeholder="添加触发表情" @keydown.enter.prevent="addReactionEmojis" /></div></el-form-item><p class="field-help">添加需要触发下载的常用表情；可点击标签右侧的 × 删除。仅监听所有已授权 Telegram 账户本人作出的匹配表情。带文件的消息会下载该消息文件；相册会作为一组下载。</p></el-form></el-card>
        <el-card shadow="never" class="settings-card"><template #header><strong>TDL 下载配置</strong></template><el-form label-position="top" @submit.prevent><el-row :gutter="12" class="download-tuning-row"><el-col :xs="24" :sm="8" :md="4"><el-form-item label="单任务线程数"><el-input-number v-model="form.download.threads" :min="1" :max="32" /></el-form-item></el-col><el-col :xs="24" :sm="8" :md="4"><el-form-item label="单任务文件并发数"><el-input-number v-model="form.download.taskLimit" :min="1" :max="16" /></el-form-item></el-col><el-col :xs="24" :sm="8" :md="4"><el-form-item label="同时执行下载任务数"><el-input-number v-model="form.download.concurrentJobs" :min="1" :max="16" /></el-form-item></el-col><el-col :xs="24" :sm="8" :md="4"><el-form-item label="连接池大小"><el-input-number v-model="form.download.poolSize" :min="0" :max="64" /></el-form-item></el-col><el-col :xs="24" :sm="8" :md="4"><el-form-item label="任务间隔（毫秒）"><el-input-number v-model="form.download.delayMs" :min="0" :max="60000" /></el-form-item></el-col></el-row>
	  <el-form-item label="下载关联评论/回复"><div style="display:flex;align-items:center;gap:12px"><el-switch v-model="form.download.includeReplies" /><span>{{ form.download.includeReplies ? '已开启' : '已关闭' }}</span></div></el-form-item>
	  <p class="field-help">频道消息默认下载关联讨论群评论；普通群组消息默认下载关联回复。此设置会在新建任务时保存，修改后不影响已创建任务。</p>
	  ...
	  <el-divider class="download-config-divider filter-config-divider" />
          ...
          <div class="field-help template-help" v-pre>
            ...
            <ul><li><code>.DialogID</code>：文件真实来源 Telegram 对话 ID</li><li><code>.DialogName</code>：文件真实来源对话名称</li><li><code>.MessageID</code>：文件真实来源消息 ID</li><li><code>.GroupedID</code>：真实 Telegram 相册组 ID；非相册消息为 <code>0</code></li><li><code>.OriginDialogName</code>：归属频道或会话名称；评论文件使用原频道名称</li><li><code>.OriginMessageID</code>：归属原帖消息 ID；原帖相册取首条消息 ID，评论继承原帖值</li><li><code>.IsComment</code>：是否来自频道评论或群组关联回复</li><li><code>.MessageText</code>：按 Telegram 客户端显示逻辑取得的消息正文</li><li><code>.FileName</code>：Telegram 原始文件名</li><li><code>.FileExt</code>：从原始文件名提取的扩展名，包含点，例如 <code>.mp4</code></li><li><code>.DownloadDate</code>：生成最终路径时的当前 Unix 时间戳（秒，可配合 <code>formatDate</code>）</li></ul>
