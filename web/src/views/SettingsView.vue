...
type Settings = {
  proxyUrl: string
  download: { threads: number; taskLimit: number; poolSize: number; delayMs: number; tempFilenameTemplate: string; finalFilenameTemplate: string }
...
<template>
  <el-container class="shell">
    <el-aside width="244px"><div class="brand">tdl <span>WEB</span></div><el-menu default-active="/settings" router><el-menu-item index="/">仪表盘</el-menu-item><el-menu-item index="/accounts">Telegram 账户</el-menu-item><el-menu-item index="/downloads">下载管理</el-menu-item><el-menu-item index="/settings">配置管理</el-menu-item></el-menu><div class="sidebar-meta"><span>上游 tdl <b>v0.20.4</b></span><span>项目版本 <b>v0.1.0</b></span></div></el-aside>
    <el-container><el-header><span>配置管理</span><el-button text @click="logout">退出管理台</el-button></el-header>
      <el-main class="settings-page"><section class="page-heading"><div><p class="eyebrow">SETTINGS</p><h1>配置管理</h1><p class="subtle">管理代理、下载与自动下载偏好。</p></div><el-button type="primary" :loading="saving" @click="save">保存配置</el-button></section>
        ...
        <el-card shadow="never" class="settings-card"><template #header><strong>TDL 下载配置</strong></template><el-form label-position="top" @submit.prevent><el-row :gutter="16"><el-col :xs="24" :sm="12" :md="6"><el-form-item label="单任务线程数"><el-input-number v-model="form.download.threads" :min="1" :max="32" /></el-form-item></el-col><el-col :xs="24" :sm="12" :md="6"><el-form-item label="同时下载任务数"><el-input-number v-model="form.download.taskLimit" :min="1" :max="16" /></el-form-item></el-col><el-col :xs="24" :sm="12" :md="6"><el-form-item label="连接池大小"><el-input-number v-model="form.download.poolSize" :min="0" :max="64" /></el-form-item></el-col><el-col :xs="24" :sm="12" :md="6"><el-form-item label="任务间隔（毫秒）"><el-input-number v-model="form.download.delayMs" :min="0" :max="60000" /></el-form-item></el-col></el-row>
