    ...
    <el-container><el-header><span>配置管理</span><el-button text @click="logout">退出管理台</el-button></el-header>
	  ...
	  <el-divider class="download-config-divider filter-config-divider" />
          ...
          <div class="field-help template-help" v-pre>
            ...
            <ul><li><code>.DialogID</code>：Telegram 对话 ID</li><li><code>.DialogName</code>：对话显示名称</li><li><code>.MessageID</code>：文件所在消息 ID</li><li><code>.GroupedID</code>：Telegram 文件组 ID；非文件组消息为 <code>0</code></li><li><code>.MessageText</code>：按 Telegram 客户端显示逻辑取得的消息正文</li><li><code>.FileName</code>：Telegram 原始文件名</li><li><code>.FileExt</code>：从原始文件名提取的扩展名，包含点，例如 <code>.mp4</code></li><li><code>.DownloadDate</code>：生成最终路径时的当前 Unix 时间戳（秒），可写成 <code>formatDate .DownloadDate "2006-01-02"</code> 取得指定格式的日期</li></ul>
