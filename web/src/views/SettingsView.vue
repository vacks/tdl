...
async function cleanupNow() { try { const result = await api<{ jobs: number; requests: number; events: number; reactionEvents: number }>('/api/maintenance/cleanup', { method: 'POST', body: JSON.stringify({ retentionDays: form.cleanup.retentionDays }) }); ElMessage.success(`已清理 ${result.jobs} 条任务历史、${result.events} 条状态事件`) } catch (error) { ElMessage.error(error instanceof Error ? error.message : '清理失败') } }
